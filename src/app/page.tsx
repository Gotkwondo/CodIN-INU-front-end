...
export default function Home() {
    return (
        <div className="max-w-7xl mx-auto p-4 sm:p-6 bg-gray-900 text-gray-100 relative">
            {/* 탭 메뉴 */}
            <nav className="fixed top-0 left-0 w-full bg-gray-800 text-gray-100 shadow-lg z-50">
                <div className="flex justify-center sm:justify-between items-center max-w-7xl mx-auto px-4 sm:px-6 py-3">
                    <div className="flex space-x-4">
                        <button
                            onClick={() => (window.location.href = "/login")}
                            className="px-4 py-2 text-sm sm:text-base bg-blue-500 hover:bg-blue-600 text-white rounded-md transition duration-200"
                        >
                            로그인
                        </button>
                        <button
                            onClick={() => (window.location.href = "/main")}
                            className="px-4 py-2 text-sm sm:text-base bg-green-500 hover:bg-green-600 text-white rounded-md transition duration-200"
                        >
                            메인 페이지
                        </button>
                    </div>
            ...
            {/* 페이지 컨텐츠 */}
            <div className="pt-16"> {/* 탭 메뉴가 상단에 고정되므로 여백 추가 */}
                {/* 헤더 섹션 */}
