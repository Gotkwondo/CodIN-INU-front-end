                ...
                <div style={{ gap: TIMETABLE_GAP }}className={`flex ml-[3px] flex-nowrap `} >
                    {RoomStatusList.map((status, index) => (
                        <button
                            ...
                            style={{ width: `${TIMETABLE_WIDTH}px`, height: `${TIMETABLE_WIDTH*2.1}px` }}
                            className={`relative shrink-0 rounded-[2.3px] ${activeIndexList[index] ? RoomStatusList[index] === 1 ? 'bg-[#17659c]' : 'bg-[#212121]' : status ? 'bg-[#0D99FF]' : 'bg-[#EBF0F7]'}`}
